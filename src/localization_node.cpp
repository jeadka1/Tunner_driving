...
private:
    virtual void onInit() {
        ...
        //sub_area_ = nhp.subscribe<std_msgs::Float32MultiArray> ("/fiducial_area_d", 1, &LocalizationNode::areaDataCallback, this);//QR fiducital_area detection
	sub_pose_dt_from_linearvel = nhp.subscribe<nav_msgs::Odometry>("/odom",30,&LocalizationNode::RobotOdomCallback,this);
        ...
        //
        pub_qr_global_update_= nhp.advertise<nav_msgs::Odometry>("/QR_localization", 1); //To use Dock in wiht Hanjeon
    ...
    void setGoal(const geometry_msgs::PoseStamped::ConstPtr& click_msg){
        ...
        if(goal_count_ > 1) {
            ...
            goal_status_changed = true;
            //event for test
            event_goal_set_[0].pose.position.x = 7;//9.7;
    ...
    void poseCallback(const ros::TimerEvent &event)
    {
        ...
        float static_x_err,static_y_err;
        cout<<"current_goal_set_[0].pose.position.x = "<<current_goal_set_[0].pose.position.x<<endl;
        cout<<"current_goal_set_[0].pose.position.x = "<<current_goal_set_[0].pose.position.y<<endl;
        cout<<"current_goal_set_[1].pose.position.x = "<<current_goal_set_[1].pose.position.x<<endl;
        cout<<"current_goal_set_[1].pose.position.x = "<<current_goal_set_[1].pose.position.y<<endl;
	...
	std::cout << "encoder-angle: "<<encoder_angle<< std::endl;
        //std::cout <<" " <<std::endl;
	...
	localization_pub.data =true;
		...
		if(STOP_cnt <STOP_MAX)
		{
		    ...
		    //turning stop
			//turning_stop_cnt++;
			//if(turning_stop_cnt>=TURNING_STOP && turning_stop_cnt <TURNING_STOP*3)
			//{
				//postech_mode = STOP_MODE;
		//	}
		//	else if(turning_stop_cnt==TURNING_STOP*3)
		//	{
				//turning_stop_cnt =0;
		//	}
			
                    //                    driving Tunnel pose
                    if(fabs(encoder_angle) > TUNNEL_ANGLE_MAX){//141592){
                        Next_step = true;
                        ROS_INFO("Tunnel POse STOP !!!!");
                    }
...
//			pub_3dlocal_start_.publish(localization_pub);
    ...
    void SetQRPose(unsigned int id){//pcw for QR local
        nav_msgs::Odometry qr_odom_msgs;

        qr_odom_msgs.header.seq = (unsigned int)id;
        qr_odom_msgs.pose.pose.position.x = 0.0;
        qr_odom_msgs.pose.pose.position.y = 0.0;
        qr_odom_msgs.pose.pose.position.z = 0.0;
        pub_qr_global_update_.publish(qr_odom_msgs);

        qr_publish_flag = false;
...
private:
    ...
    ros::Publisher pub_qr_global_update_;
    ...
    float fid_area=0;
    float prev_fid_ID =0.0;
    bool qr_publish_flag = true;
    bool Camera_noline_flag= false;
	...
	unsigned int  turning_stop_cnt =0;
...
void LocalizationNode::areaDataCallback(const std_msgs::Float32MultiArray::ConstPtr& area_msgs) //QR detection Aurco realsense for a front camera.
{
    fid_ID = (unsigned int) area_msgs->data[0];
    fid_area =area_msgs->data[1];
    //ROS_INFO("fid_ID: %d, are: %f",fid_ID, fid_area);
    if(!qr_publish_flag && prev_fid_ID != fid_ID) // Not to publish everytime whenever ID is detected
    {
        qr_publish_flag = true;
    }
    if(fid_area >= QR_DISTANCE && qr_publish_flag)
        SetQRPose(fid_ID);
